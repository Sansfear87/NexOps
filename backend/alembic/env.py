...
# Use configured URL or fallback to environment/dynamic URL
current_url = config.get_main_option("sqlalchemy.url")
if not current_url or "devops_user:devops_password@localhost" in current_url:
    dynamic_url = get_sync_database_url()
    if dynamic_url:
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        context.configure(
            ...
            target_metadata=target_metadata,
            render_as_batch=True if "sqlite" in (url or "") else False
        )
